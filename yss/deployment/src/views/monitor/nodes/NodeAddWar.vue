<template>
    <el-dialog title="添加War包" :visible.sync="showDialog" width="580px"  :close-on-click-modal="false" :close-on-press-escape="false" append-to-body>
      <el-tabs type="card" v-model="activeName">
        <el-tab-pane label="本地文件" name="local">
          <div>
            <el-form label-position="right" label-width="130px" :inline="true" style="height: 140px;" ref="formAddWar">
              <el-form-item label="War文件" prop="fileList">
                <el-upload
                  class="upload-demo"
                  action
        ...
        <el-tab-pane label="远程文件" name="remote">
          <div>
            <el-table :data="remote.tableData" border style="margin: 10px 20px 20px 20px; width:540px"  max-height="375" :header-cell-style="{background:'#F5F7FA',color:'#606266'}" @row-click="handleClickRow" v-loading="loadingData">
              ...
              <el-table-column property="path" label="war包目录"></el-table-column>
            </el-table>
          </div>
        </el-tab-pane>
      </el-tabs>
...
export default {
  ...
  methods: {
    customUpload(file) {
      ...
      this.fileData.append('tomcatId', this.tomcatId);
      return false;
    },
    execAdd() {
      if (this.activeName === 'local') {
        if (!this.fileList || this.fileList.length === 0) {
          this.$message.error({message: '请选择一个War文件'});
          return;
        }
      } else {
        if (!this.remote.selectPath || this.remote.selectPath === '') {
          this.$message.error({message: '请选择一个War文件路径'});
          return;
        }
      }
      this.showLoading();
      ...
      this.onNodeWarAdded(api.uploadNodeWar(this.fileData), this.index);
    },
    execListWar() {
      this.loadingData = true;
      api.getWarListOfNode(this.nodeId)
        .then(res => {
