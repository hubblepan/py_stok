<template>
  <div style="margin-left: 20px; margin-top: 30px;">
    <el-form label-position="left" label-width="120px" :model="formData" style="height: 120px;" ref="formMain">
      <el-form-item label="部署类型:" v-if="false">
        <el-checkbox v-model="formData.distributed">分布式</el-checkbox>
      </el-form-item>
      <el-form-item label="文件类型:">
        <el-radio-group v-model="formData.fileType">
<!--          <el-radio :label="true">手工上传</el-radio>-->
<!--          <el-radio :label="false">远程文件</el-radio>-->
      ...
      <el-form-item label="部署包路径:" prop="fileDir" v-if="formData.fileType === '3'">
        <el-input v-model="formData.fileDir" placeholder="输入一个部署包路径或者从服务器中选择一个部署包" style="width: 360px;" size="small"></el-input>
        &nbsp;或
        <el-button @click.prevent="handleUploadRemote" type="primary" size="small" class="ml-2">从服务器选择</el-button>
      </el-form-item>
      <el-form-item label="部署包文件:" prop="fileList" v-if="formData.fileType === '2'">
        <el-upload
          class="upload-demo"
      ...
      <el-form-item label="部署包名称:" prop="fileRepo" v-if="formData.fileType === '1'">
        <el-input v-model="formData.fileRepo" placeholder="输入一个部署包路径或者从版本仓库中选择一个部署包" style="width: 360px;" size="small"></el-input>
        &nbsp;或
        <el-button @click.prevent="handleUploadRepo" type="primary" plain size="small" class="ml-2">版本仓库</el-button>
      </el-form-item>
    </el-form>
    <node-add-war-remote :on-added="onAdded" ref="UploadRemote"></node-add-war-remote>
    <node-deploy-upload-repo :on-added="onRepoAdded" ref="UploadRepo"></node-deploy-upload-repo>
  </div>
</template>
...
import NodeDeployUploadRepo from './NodeDeployUploadRepo';
import * as api from '../api/node_deploy_api';

export default {
  name: 'NodeDeployUpload',
  components: {NodeDeployUploadRepo, NodeAddWarRemote},
  props: ['type'],
  data() {
    return {
      nodeId: '',
    ...
    validForm() {
      if (this.formData.fileType === '2') {
        ...
        if (this.formData.fileRepo === '') {
          this.$alert('请选择一个部署包版本', {type: 'error'});
          return false;
        }
        return true;
      }
