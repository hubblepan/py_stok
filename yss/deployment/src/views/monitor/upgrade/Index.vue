<template>
  <div id="tem-index">
    <div style="height: 60px; font-size: 18px; line-height: 60px; margin-left: 20px;">开始应用升级</div>
    ...
    <div>
      ...
      <div class="tem-step-content">
        <div ref="ctn">
          <upload
            ref="Upload"
            v-if="currentStepName === 'Upload'"
            @modifyTomcatDir="modifyTomcatDir"
            :tomcatDir="tomcatInfo.tomcatDir"
          ></upload>
          <compare
            ref="Compare"
...
export default {
  ...
  created() {
    ...
    this.tomcatInfo = this.$route.params.tomcatInfo;
  },
  mounted() {
  ...
  methods: {
    ...
    next(name) {
      ...
      if (name === 'Complete') {
        if (!this.$refs.Compare.isValid()) {
          this.$message.error('文件对比失败, 无法继续下一步');
        ...
        //     // 进行对话框提示
        //     this.$confirm('当前Tomcat正在运行, 是否停止Tomcat?', '提示', {
