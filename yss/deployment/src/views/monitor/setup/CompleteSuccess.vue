<template>
  <div class="tem-success" v-loading="configInfoLoading">
    <el-scrollbar id="scrollbar" ref="scrollbar" v-bind:style="{height: previewHeight + 'px'}" v-if="showBtnDeploy">
        <div v-bind:style="{height: previewHeight + 'px'}">
        ...
        <div style="text-align: center; margin-top: 20px;">
          <el-button @click="handleWsConnect" type="primary" v-show="showBtnDeploy">确认部署</el-button>
        </div>
      </div>
    </el-scrollbar>
...
export default {
  ...
  methods: {
    ...
    async showData() {
      await api.previewAllConfig([this.$parent.tomcatSelected.id])
        .then(res => {
          if (res.success) {
            this.data = res.data[0];
          } else {
            this.$message.error('预览配置查询失败。' + res.msg);
            return false;
          }
        })
        .catch(reason => {
    ...
    handleWsConnect() {
      ...
      api.connectInfo([this.$parent.tomcatSelected.id])
        .then(res => {
    ...
    deploy: function() {
      ...
      this.configInfoLoading = true;
      api
        .deploySingle([this.$parent.tomcatSelected.id])
        .then((result) => {
