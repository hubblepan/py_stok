  ...
  <el-dialog :title="title" :visible.sync="showDialog" width="820px"  :close-on-click-modal="false" :close-on-press-escape="false" append-to-body>
    <div>
      <div style="display: inline-block; margin-left: 10px">
        <el-form :model="formData" :rules="rules" id="Form" label-width="100px" size="mini" ref="configForm" label-position="left">
          <el-form-item label="历史配置版本" prop="type">
            <el-select v-model="formData.oldConfigVersionId" placeholder="请选择要对比的历史配置" @change="handleCompare">
      ...
      <div style="clear: right; margin-left: 10px; margin-right: 10px">
        <div class="tree-head">
          ...
          <span class="head-size">版本1</span>
          <span class="head-modified">当前版本</span>
        </div>
        <el-tree
          :data="leftData"
          :props="defaultProps"
...
export default {
  ...
  methods: {
    handleCompare(id) {
      ...
      api.compareInstanceVersionConfig(id, this.instanceConfigVersionVo)
        .then(res => {
          this.loadingLeft = false;
          res.data && (this.leftData = res.data.children);
        })
        .catch(reason => {
          this.loadingLeft = false;
          console.log(reason);
    ...
    show(instanceConfigVersionVo) {
      this.showDialog = true;
      this.title = '配置对比';
      this.instanceConfigVersionVo = instanceConfigVersionVo;
      this.leftData = [];
      this.formData = {
        oldConfigVersionId: '',
      };
      this.initData(instanceConfigVersionVo.instanceId);
    },
    initData(instanceId) {
