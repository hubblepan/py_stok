...
import TaskDetailHandle from '../handles/TaskDetailHandle';

const { TabPane } = Tabs;

const TaskDetailModal = (props) => {
  const detailModel0 = useModel('ocp.monitorLogSummary.taskDetail0');
  const detailModel1 = useModel('ocp.monitorLogSummary.taskDetail1');
  const detailModel2 = useModel('ocp.monitorLogSummary.taskDetail2');
  // handle不能用useMemo
  const detailHandle0 = new TaskDetailHandle(detailModel0);
  const detailHandle1 = new TaskDetailHandle(detailModel1);
  const detailHandle2 = new TaskDetailHandle(detailModel2);
  ...
  useEffect(() => {
    if (visible) {
      ...
      console.log(id);
      detailHandle0.query({ params: { mode: 'all' } });
      detailHandle1.query({ params: { mode: 'success' } });
      detailHandle2.query({ params: { mode: 'fail' } });
    } else {
      setMode('1');
      // 将页码重置为1
