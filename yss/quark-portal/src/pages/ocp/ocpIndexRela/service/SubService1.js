...
export default class MasterService0 extends BaseService {
  constructor() {
    super({
      ...
      query: `/${YSSMONITOR}/YSSUCOBRIDGE/ws/com/yss/uco/monitor/support/indexRela/controller/IMonitorIndexRelaServiceController/queryIndexModeData?serviceId=yssmonitor-testor`,
      ...
      refresh: `/${YSSMONITOR}/YSSUCOBRIDGE/ws/com/yss/uco/monitor/support/indexRela/controller/IMonitorIndexRelaServiceController/queryIndexModeDataByRefresh?serviceId=yssmonitor-testor`,
    });
  }
      ...
      request(this.url.query, {
        ...
        .then((response) => {
          ...
          data.dataList.forEach((item) => {
            /** 1.拼接code字段作为主键 */
            item.code = item.indexCode + (item.portCode || '');
            /** 2.子节点添加pKey字段。注意，pKey和key不能是一个字段 */
            if (item.portCode) {
              item.indexCodeP = item.indexCode;
            }
          });
